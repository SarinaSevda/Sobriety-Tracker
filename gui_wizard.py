...
class Gui_Wizard:   #******NEU*******
    def __init__(self, root, user_data: UserData):
        ...
        #Schritt 2: Sucht
        self.addiction_label = ttk.Label(root, text="Wähle deine Süchte aus:", **ADDICTION_LABEL_STYLE)
        ...
        # Schritt 3: Ziele
        self.goal_label = ttk.Label(self.root, text="Was möchtest du erreichen?", **GOAL_LABEL_STYLE)
        self.goal_var = tk.StringVar(value="Bitte auswählen")
        self.goal_dropdown = ttk.OptionMenu(self.root, self.goal_var, *GOAL_OPTIONS)
        self.goal_button = ttk.Button(self.root, text="Weiter", command=self.submit_goal)
        self.back_button_3 = ttk.Button(self.root, text="Zurück", command=self.ask_addiction)

        # Schritt 4: Startzeitpunkt
        self.sobriety_label = ttk.Label(self.root, text="Seit wann bist du nüchtern?", **SOBRIETY_LABEL_STYLE)
        self.sobriety_date = DateEntry(self.root, width=12, borderwidth=2, date_pattern='dd.mm.yy', locale='de_DE')

        self.sobriety_time_label = ttk.Label(self.root, text="Super! Um welche Uhrzeit?", **SOBRIETY_LABEL_STYLE)
        ...
        self.sobriety_button = ttk.Button(self.root, text="Weiter", command=self.submit_sobriety_duration)
    ...
    def submit_name(self):
        ...
        if name:
            ...
            self.user_data.load_from_db()  # ***wenn ich hier die Option zum Abruf von Nutzerdaten will - lade alte Daten
            self.ask_addiction()
        else:
    ...
    def open_new_addiction_choice(self):
        ...
        new_dropdown = ttk.OptionMenu(self.addiction_frame, new_var, ADDICTION_OPTIONS[0], *ADDICTION_OPTIONS)
    ...
    def submit_addiction(self):
        ...
        if addictions:
            self.user_data.addiction = addictions
            self.ask_goal()
        else:
